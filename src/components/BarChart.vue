...
export default {
  ...
  mounted: function () {
    ...
    let x = (this.$_.isDate(this.domain[0]) ? this.$d3.scaleTime() : this.$d3.scaleLinear())
      .domain(this.domain)
      .rangeRound([0, width])
    let y = this.$d3.scaleLinear()
      .domain([0, this.group.top(1)[0].value])
      .range([height, 0])
    let axis = this.$d3.axisBottom()
      .scale(x)
    let brush = this.$d3.brushX(x)
      .extent([[0, 0], [width, height]])

    let g = this.$d3.select(this.$el).html('')
      .append('svg')
    ...
    brush.on('brush', function () {
      let selection = self.$d3.brushSelection(this)
      gClipRect
        .attr('x', selection[0])
