...
namespace('org', function () {
  desc('Delete all test events')
  task('delete', {async: true}, function () {
    request
      .get('https://api.teambition.com/api/projects/5ba704fd0575fd00185b74b7/events')
      .query({
        startDate: new Date('2016.08.01'),
        access_token: process.env.ACCESS_TOKEN
      })
  ...
  task('create', function () {
    ...
    request
      ...
      .query({ access_token: process.env.ACCESS_TOKEN })
      .send({
        _projectId: '5ba704fd0575fd00185b74b7',
        title: '沐浴更衣',
        startDate: start.toISOString(),
        endDate: end.toISOString(),
        tagIds: ['5ba7945e9c9dbc001861b7d0']
      })
      .end()
